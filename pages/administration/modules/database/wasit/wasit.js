...
const tbody = document.getElementById("tbodyWasit");
...
function render(){
  tbody.innerHTML = data.map((row,i)=>`
    <tr class="${selectedIndex===i?'active-row':''}" onclick="window.selectRowWasit(${i})">
      <td>${i+1}</td>
      <td>${row.kode}</td>
      <td>${row.nama}</td>
      <td>${row.lisensi}</td>
      <td>${row.wa}</td>
      <td>
        <button onclick="event.stopPropagation();window.editWasit(${i})">Edit</button>
        <button onclick="event.stopPropagation();window.deleteWasit(${i})">Hapus</button>
      </td>
    </tr>
  `).join("");
}
...
window.selectRowWasit = function(index){
  selectedIndex = index;
  render();
  renderDetail();
};
...
function openAddModal(){
  openFormModal("Tambah Wasit",{},async(form)=>{
...
window.editWasit = function(index){
  openFormModal("Edit Wasit",data[index],async(form)=>{
...
window.deleteWasit = function(index){
...
function downloadTemplate(){
  const header="NO,KODE,GENDER,NAMA,NIK,LISENSI,WA,ALAMAT,BANK,REKENING";
  const blob=new Blob([header],{type:"text/csv"});
  const a=document.createElement("a");
  a.href=URL.createObjectURL(blob);
  a.download="template-wasit.csv";
  a.click();
}
...
function openFormModal(title,values,onSave){
  ...
  <div class="modal-overlay">
    <div class="modal-card">
      ...
      <input id="m_lisensi" placeholder="Lisensi" value="${values.lisensi||""}">
  ...
  document.getElementById("saveBtn").onclick=()=>{
    const form={
      ...
      lisensi:document.getElementById("m_lisensi").value,
